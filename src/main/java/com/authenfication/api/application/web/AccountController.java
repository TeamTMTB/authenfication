...
public class AccountController {
    ...
    public Account findUserById(@ApiParam(value = "언어", defaultValue = "ko") @RequestParam String lang) {
        // SecurityContext에서 인증받은 회원의 정보를 얻어온다.
        Authentication authentication = SecurityContextHolder.getContext().getAuthentication();
        String id = authentication.getName();
        // 결과데이터가 단일건인경우 getSingleResult를 이용해서 결과를 출력한다.
        return accountRepository.findByAccountId(id).orElseThrow(CUserNotFoundException::new);
    }
    ...
    public String delete(
        ...
        accountRepository.deleteById(msrl);
